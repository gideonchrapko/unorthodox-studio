...
function getVisual() {
	return (dispatch) => {
		sanityClient.fetch(`*[_type == "visualProject"]|order(orderRank){ slugRoute, projectImages, projectTitle, clients, "playbackId": videoPost.video.asset->playbackId }`).then((data) => {
			dispatch({
				type: FETCH_VISUAL,
				payload: data,
			})
...
function getSound() {
	return (dispatch) => {
		sanityClient.fetch(`*[_type == "soundProject"]|order(orderRank){ slugRoute, projectImages, projectTitle, clients, "playbackId": videoPost.video.asset->playbackId }`).then((data) => {
			dispatch({
				type: FETCH_SOUND,
				payload: data,
			})
...
function getFashion() {
	return (dispatch) => {
		sanityClient.fetch(`*[_type == "fashionProject"]|order(orderRank){ slugRoute, projectImages, projectTitle, clients, "playbackId": videoPost.video.asset->playbackId }`).then((data) => {
			dispatch({
				type: FETCH_FASHION,
				payload: data,
			})
...
function getUX() {
	return (dispatch) => {
		sanityClient.fetch(`*[_type == "uxProject"]|order(orderRank){ slugRoute, projectImages, projectTitle, clients, "playbackId": videoPost.video.asset->playbackId }`).then((data) => {
			dispatch({
				type: FETCH_UX,
				payload: data,
			})
